...
@scenario("features/hardware/email_to_order.feature", "Send Order to email")
def test_ETO_order_mail():
    pass


@scenario("features/hardware/email_to_order.feature", "Search ETO order")
def test_ETO_order_search():
    pass
...
@when(parsers.parse('mail send with order pdf'))
def send_email_for_order(init_driver):
    feature_file_name = "email_to_order"
    email_to_order_steps = ValidateEmailToOrderData(init_driver)
    try:
        if not email_to_order_steps.send_email_for_order(feature_file_name, screen_shot):
...
@when(parsers.parse('Search ETO order by order status completed'))
def do_eto_select_order_by_status(init_driver):
    feature_file_name = "email_to_order"
    email_to_order_steps = ValidateEmailToOrderData(init_driver)
    try:
        if not email_to_order_steps.do_eto_select_order_by_status(feature_file_name, screen_shot):
            raise Exception("Failed to select completed ETO order")
    except Exception as e:
        logger.error("Error while selecting completed ETO order %s" + str(e))
        raise e


@when(parsers.parse('Search ETO order by Customer PO Number "{scenario_number}"'))
def do_eto_select_order_by_PO(init_driver, scenario_number):
    feature_file_name = "email_to_order"
    email_to_order_steps = ValidateEmailToOrderData(init_driver)
    try:
        if not email_to_order_steps.do_eto_select_order_by_PO(feature_file_name, screen_shot, scenario_number):
            raise Exception("Failed to select ETO order based on Customer PO number")
    except Exception as e:
        logger.error("Error while selecting ETO order based on Customer PO number %s" + str(e))
        raise e


#
@then(parsers.parse('verify ETO order status'))
def verify_ETO_order_status(init_driver):
    feature_file_name = "email_to_order"
    email_to_order_steps = ValidateEmailToOrderData(init_driver)
    try:
        if not email_to_order_steps.verify_ETO_order_status(feature_file_name, screen_shot):
            raise Exception("Failed to verify ETO order status")
    except Exception as e:
        logger.error("Error while verifying ETO order page status %s" + str(e))
        raise e

...
class DataCreationViaApi:
    ...
    def get_request_for_x4d_order_exception_order_info(self, confirmation_id):
        error_order_base_url = ApiBaseURLs.error_order_base_url+"/"+confirmation_id
        path = ".\\RestApi\\Resources\\x4d_data_error_order_Yes_shiping_details_payload.json"
        self.logger.info("Get Url: " + error_order_base_url)
        headers = apiHeaderParam.headers
        data = self.common_json.read_json_file(path)
        self.logger.info(data)
        response = requests.get(error_order_base_url, json=data, headers=headers)
        json_data = response.json()
        self.logger.info(response.status_code)
        assert response.status_code == 200
        self.logger.info("json POST response body: ", json_data)
    ...
    def post_create_request_for_x4d_order_exception(self):
        error_order_base_url = ApiBaseURLs.error_order_base_url
        path = ".\\RestApi\\Resources\\x4d_data_error_order_Yes_shiping_details_payload.json"
        self.logger.info("post url: " + error_order_base_url)
        headers = apiHeaderParam.headers
        data = self.common_json.read_json_file(path)
        data['customerOrderNumber'] = "X4D112_1"
        self.logger.info(data)
        response = requests.post(error_order_base_url, json=data, headers=headers)
        json_data = response.json()
        self.logger.info(response.status_code)

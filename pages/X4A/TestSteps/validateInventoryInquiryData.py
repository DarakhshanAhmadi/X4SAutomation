...
class ValidateInventoryInquiryData:
    ...
    def validate_product_details_fields(self, sku_data, feature_file_name, screen_shot):
        x4a_inventory_inquiry = X4AInventoryInquiryPage(self.driver)
        try:
            breakpoint()
            product_details_sections_headers, description_section, vendor_details_section, codes_section, categories_section = x4a_inventory_inquiry.get_product_details()
            assert len(self.product_details_sections) == len(
                product_details_sections_headers), 'Number of section under product details mismatched'
